import { PLAYER } from '../../protocol/index.js';
...
export function createGroundItem(weaponId, position) {
    nextItemNumber += 1;
    return { id: `g${nextItemNumber}`, weaponId, position: { ...position }, lockedMs: 400 };
}
export function toSnapshot(item) {
    return { id: item.id, x: item.position.x, y: item.position.y, weaponId: item.weaponId };
}
